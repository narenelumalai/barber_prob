...
func customerArrives() {
	...
	case waitingRoom <- struct{}{}:
		...
		select {
		case barberChair <- struct{}{}: //will receive the barber signals
			...
			<-barberChair //will send barber chair signals
		default:
			fmt.Println("All barber chairs are occupied, customer waiting.")
			<-waitingRoom //will send waiting room signals
		}
	default:
